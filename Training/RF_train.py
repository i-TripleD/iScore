...
for i in range(1):
    ...
    for train, test in kf.split(x_train, y_train):    
        ...
        RFmodel = RandomForestRegressor(n_estimators=200, verbose=1, criterion='squared_error', min_samples_split=2, oob_score=True, n_jobs=-1)
        RFmodel.fit(x_train[train], y_train[train])

        pred = RFmodel.predict(x_train[test])


        filename  = 'Models/models_RF/model_' + str(i) + '_' + str(fold) + '.joblib'
...
def combine_csv_files(directory, output_file):
    ...
    combined_df.columns = ['exp', 'RF']
    combined_df.to_csv(output_file, index=False)
    print(f"Combined CSV created at {output_file}")

directory = 'Models/models_RF'  
output_file = 'Models/models_RF/combined_RF.csv'  
